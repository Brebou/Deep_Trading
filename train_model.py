    ...
    for epoch in range(nepochs):
        ...
        for k in range(0, dataset_size, batch_size):
            ...
            if k == 0:
                ...
                continue
            inp = torch.from_numpy(train_dataset[k-batch_size:k]).to(torch.float32)
            label = torch.from_numpy(train_labels[k - batch_size + 1 : k + 1]).to(torch.float32)
            ...
            predictions = (predictions == label).int()

            # Computing accuracy
            train_accuracy += float(predictions.sum() / dataset_size)
        ...
        for k in range(0, dataset_size, batch_size):
            with torch.no_grad():
                ...
                if k == 0:
                    ...
                    continue
                inp = torch.from_numpy(test_dataset[k-batch_size:k]).to(torch.float32)
                label = torch.from_numpy(test_labels[k - batch_size + 1 : k + 1]).to(torch.float32)
                ...
                # Computing accuracy
                test_accuracy += float(predictions.sum() / dataset_size)
